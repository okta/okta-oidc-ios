/*! @file OKTAuthorizationRequest.m
 ...
 */
static NSString *const OKTOAuthUnsupportedResponseTypeMessage =
 ...
 */
NSString *const OKTOAuthorizationRequestCodeChallengeMethodS256 = @"S256";

@implementation OKTAuthorizationRequest
...
{
  NSString *codeIdToken = [@[OKTResponseTypeCode, OKTResponseTypeIDToken]
                           componentsJoinedByString:@" "];
  NSString *idTokenCode = [@[OKTResponseTypeIDToken, OKTResponseTypeCode]
                           componentsJoinedByString:@" "];

  return [responseType isEqualToString:OKTResponseTypeCode]
    ...
    additionalParameters:(nullable NSDictionary<NSString *, NSString *> *)additionalParameters
{
  ...
  if (self) {
    ...
    if (![[self class] isSupportedResponseType:_responseType]) {
      NSAssert(NO, OKTOAuthUnsupportedResponseTypeMessage, _responseType);
      return nil;
    }
    _state = [state copy];
    _nonce = [nonce copy];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
  OKTServiceConfiguration *configuration =
      [aDecoder decodeObjectOfClass:[OKTServiceConfiguration class]
...
+ (nullable NSString *)generateCodeVerifier {
  return [OKTTokenUtilities randomURLSafeStringWithSize:kCodeVerifierBytes];
}
...
+ (nullable NSString *)codeChallengeS256ForVerifier:(NSString *)codeVerifier {
  ...
  NSData *sha256Verifier = [OKTTokenUtilities sha256:codeVerifier];
  return [OKTTokenUtilities encodeBase64urlNoPadding:sha256Verifier];
}
