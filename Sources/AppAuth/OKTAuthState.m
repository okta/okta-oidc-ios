...
/*! @brief Object to hold OKTAuthState pending actions.
 */
@interface OKTAuthStatePendingAction : NSObject
@property(nonatomic, readonly, nullable) OKTAuthStateAction action;
@property(nonatomic, readonly, nullable) dispatch_queue_t dispatchQueue;
@end
@implementation OKTAuthStatePendingAction
- (id)initWithAction:(OKTAuthStateAction)action andDispatchQueue:(dispatch_queue_t)dispatchQueue {
  self = [super init];
  if (self) {
    _action = action;
...
/*! @brief The access token generated by the authorization server.
    @discussion Rather than using this property directly, you should call
        @c OKTAuthState.withFreshTokenPerformAction:.
 */
...
/*! @brief ID Token value associated with the authenticated session.
    @discussion Rather than using this property directly, you should call
        OKTAuthState.withFreshTokenPerformAction:.
 */
...
- (nonnull instancetype)init
    OKT_UNAVAILABLE_USE_INITIALIZER(@selector(initWithAuthorizationResponse:tokenResponse:))
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p, isAuthorized: %@, refreshToken: \"%@\", "
                                    ...
                                    _scope,
                                    [OKTTokenUtilities redact:self.accessToken],
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
  _lastAuthorizationResponse = [aDecoder decodeObjectOfClass:[OKTAuthorizationResponse class]
                                                      forKey:kLastAuthorizationResponseKey];
  _lastTokenResponse = [aDecoder decodeObjectOfClass:[OKTTokenResponse class]
...
- (void)updateWithAuthorizationResponse:(nullable OKTAuthorizationResponse *)authorizationResponse
  ...
  if (error.domain == OKTOAuthAuthorizationErrorDomain) {
...
- (void)updateWithTokenResponse:(nullable OKTTokenResponse *)tokenResponse
  ...
  if (_authorizationError) {
    ...
    NSLog(@"OKTAuthState:updateWithTokenResponse should not be called in an error state [%@] call"
  ...
  // If the error is an OAuth authorization error, updates the state. Other errors are ignored.
  if (error.domain == OKTOAuthTokenErrorDomain) {
    [self updateWithAuthorizationError:error];
    return;
  }
  if (!tokenResponse) {
...
- (OKTTokenRequest *)tokenRefreshRequestWithAdditionalParameters:
...
- (void)performActionWithFreshTokens:(OKTAuthStateAction)action
  ...
  if (!_refreshToken) {
    ...
    NSError *tokenRefreshError = [
      OKTErrorUtilities errorWithCode:OKTErrorCodeTokenRefreshError
                      underlyingError:nil
  ...
  OKTAuthStatePendingAction* pendingAction =
      [[OKTAuthStatePendingAction alloc] initWithAction:action andDispatchQueue:dispatchQueue];
  ...
  [OKTAuthorizationService performTokenRequest:tokenRefreshRequest
    ...
    for (OKTAuthStatePendingAction* actionToProcess in actionsToProcess) {
