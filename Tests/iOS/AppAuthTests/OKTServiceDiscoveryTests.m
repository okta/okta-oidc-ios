...
@implementation OKTServiceDiscoveryTests
...
- (void)testURLs {
  ...
  OKTServiceDiscovery *discovery =
      [[OKTServiceDiscovery alloc] initWithDictionary:serviceDiscoveryDictionary
                                                error:&error];
...
- (void)testErrorWhenBadFormat {
  NSError *error;
  OKTServiceDiscovery *discovery = [[OKTServiceDiscovery alloc] initWithJSON:@"JUNK" error:&error];
  ...
  XCTAssertEqualObjects(error.domain, OKTGeneralErrorDomain, @"");
  XCTAssertEqual(error.code, OKTErrorCodeJSONDeserializationError, @"");
}
...
- (void)testErrorWhenRootObjectNotNSDictionary {
  NSError *error;
  OKTServiceDiscovery *discovery = [[OKTServiceDiscovery alloc] initWithJSON:kDiscoveryDocumentNotDictionary error:&error];
  XCTAssertNil(discovery, @"When initializing a discovery document, it should not return an "
               "instance if the root JSON object is not a NSDictionary.");
  XCTAssertNotNil(error, @"There should be an error indicating we received bad JSON.");
  XCTAssertEqualObjects(error.domain, OKTGeneralErrorDomain, @"");
  XCTAssertEqual(error.code, OKTErrorCodeInvalidDiscoveryDocument, @"");
}
...
- (void)testErrorWhenMissingFields {
  NSError *error;
  OKTServiceDiscovery *discovery =
      [[OKTServiceDiscovery alloc] initWithDictionary:@{ } error:&error];
  XCTAssertNil(discovery, @"When initializing a discovery document, it should not return an "
...
- (void)testErrorWhenMissingFieldsJSON {
  NSError *error;
  OKTServiceDiscovery *discovery =
      [[OKTServiceDiscovery alloc] initWithJSON:kDiscoveryDocumentMissingField error:&error];
...
/*! @brief Tests that we do not get an error, and we do get an instance of
        @c OKTServiceDiscovery when the required fields are in the source dictionary.
 */
...
- (void)pendingTestErrorWhenMalformedFields {
  ...
  OKTServiceDiscovery *discovery =
      [[OKTServiceDiscovery alloc] initWithDictionary:serviceDiscoveryDictionary error:&error];
...
- (void)pendingTestErrorWhenJSONNullField {
  NSError *error;
  OKTServiceDiscovery *discovery =
      [[OKTServiceDiscovery alloc] initWithJSON:kDiscoveryDocumentNullField error:&error];
  XCTAssertNil(discovery, @"When initializing a discovery document, it should not return an "
                          "instance if there are missing required fields.");
  XCTAssertNotNil(error, @"There should be an error indicating we are missing required fields.");
  XCTAssertEqualObjects(error.domain, OKTGeneralErrorDomain, @"");
...
- (void)testJSONStringDecoding {
  NSError *error;
  OKTServiceDiscovery *discovery =
      [[OKTServiceDiscovery alloc] initWithJSON:kDiscoveryDocument error:&error];
...
- (void)testJSONDataDecoding {
  ...
  OKTServiceDiscovery *discovery =
      [[OKTServiceDiscovery alloc] initWithJSONData:jsonData error:&error];
  XCTAssertNotNil(discovery, @"When initializing a discovery document with JSON it should return a"
                              "valid object");
  XCTAssertNil(error, @"There should not be any errors.");
  XCTAssertEqualObjects(discovery.authorizationEndpoint,
...
- (void)pendingTestJSONEqualsDictionary {
  NSError *error;
  OKTServiceDiscovery *discovery =
    [[OKTServiceDiscovery alloc] initWithDictionary:[[self class] minimumServiceDiscoveryDictionary]
                                              error:&error];
  ...
  OKTServiceDiscovery *discovery2 =
      [[OKTServiceDiscovery alloc] initWithJSONData:jsonData error:&error];
...
- (void)testSecureCoding {
  ...
  OKTServiceDiscovery *unarchived = [NSKeyedUnarchiver unarchiveObjectWithData:data];
...
- (void)testCopying {
  ...
  OKTServiceDiscovery *unarchived = [discovery copy];
...
/*! @define TestFieldBackedBy
    @brief Auto-generates unit test methods for checking that a property in
        @c OKTServiceDiscovery is, in fact, using the key specified by the second
...
- (void)testField_##_field_ {                                                                      \
  ...
  OKTServiceDiscovery *discovery =                                                                 \
      [[OKTServiceDiscovery alloc] initWithDictionary:json error:&error];                          \
  XCTAssertNotNil(discovery, @"We supplied the minimum required fields when initializing the "     \
                             "service discovery instance, so we should have gotten a new "         \
                             "instance.");                                                         \
  XCTAssertNil(error, @"We supplied the minimum required fields when initializing the "            \
