/*! @file OKTAuthorizationRequestTests.m
...
#import "OKTAuthorizationRequestTests.h"
...
@implementation OKTAuthorizationRequestTests

+ (NSString *)codeChallenge {
  return [OKTAuthorizationRequest codeChallengeS256ForVerifier:kTestCodeVerifier];
}
+ (NSString *)codeChallengeMethod {
  return OKTOAuthorizationRequestCodeChallengeMethodS256;
}

+ (OKTAuthorizationRequest *)testInstance {
  NSDictionary *additionalParameters =
      @{ kTestAdditionalParameterKey : kTestAdditionalParameterValue };
  OKTServiceConfiguration *configuration = [OKTServiceConfigurationTests testInstance];
  OKTAuthorizationRequest *request =
      [[OKTAuthorizationRequest alloc] initWithConfiguration:configuration
                  ...
                  clientSecret:kTestClientSecret
                         scope:[OKTScopeUtilities scopesWithArray:@[ kTestScope, kTestScopeA ]]
 ...
 */
- (void)testCopying {
  OKTAuthorizationRequest *request = [[self class] testInstance];
  ...
  OKTAuthorizationRequest *requestCopy = [request copy];
...
- (void)testSecureCoding {
  ...
  OKTAuthorizationRequest *requestCopy = [NSKeyedUnarchiver unarchiveObjectWithData:data];
...
- (void)testPKCEVerifierCompliance {
  ...
  for (int i = 0; i < 1000; i++) {
    NSString *codeVerifier = [OKTAuthorizationRequest generateCodeVerifier];
...
- (void)testSupportedResponseTypes {
  ...
  NSString *scope = [OKTScopeUtilities scopesWithArray:@[ kTestScope, kTestScopeA ]];
