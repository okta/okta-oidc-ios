...
/*! @brief A UI Coordinator for testing, has no user agent and doesn't support user interaction.
        ...
        the response.
 */
@interface OKTAuthorizationUICoordinatorNonInteractive : NSObject <OKTExternalUserAgent, NSURLSessionTaskDelegate>{
  NSURLSession *_urlSession;
  __weak id<OKTExternalUserAgentSession> _session;
}
@end

@implementation OKTAuthorizationUICoordinatorNonInteractive
...
@interface OKTRPProfileCode : XCTestCase {
  // private variables
  OKTAuthorizationUICoordinatorNonInteractive *_coordinator;
  FILE * _logFile;
}
typedef void (^PostRegistrationCallback)(OKTServiceConfiguration *configuration,
                                         OKTRegistrationResponse *registrationResponse,
                                         NSError *error
                                         );

typedef void (^CodeExchangeCompletion)(OKTAuthorizationResponse *_Nullable authorizationResponse,
                                       OKTTokenResponse *_Nullable tokenResponse,
                                       NSError *tokenError
                                       );

typedef void (^UserInfoCompletion)(OKTAuthState *_Nullable authState,
...
@implementation OKTRPProfileCode
...
- (void)doRegistrationWithIssuer:(NSURL *)issuer callback:(PostRegistrationCallback)callback {
  ...
  // discovers endpoints
  [OKTAuthorizationService discoverServiceConfigurationForIssuer:issuer
      completion:^(OKTServiceConfiguration *_Nullable configuration, NSError *_Nullable error) {
    ...
    OKTRegistrationRequest *request =
        [[OKTRegistrationRequest alloc] initWithConfiguration:configuration
    ...
    [OKTAuthorizationService performRegistrationRequest:request
                                               delegate:nil
                                             completion:^(OKTRegistrationResponse *_Nullable regResp, NSError *_Nullable error) {
      if (regResp) {
...
- (void)codeFlowWithExchangeForTest:(NSString *)test completion:(CodeExchangeCompletion)completion {
  [self codeFlowWithExchangeForTest:test scope:@[ OKTScopeOpenID ] completion:completion];
}
...
- (void)codeFlowWithExchangeForTest:(NSString *)test
  ...
  [self doRegistrationWithIssuer:issuer callback:^(OKTServiceConfiguration *configuration,
                                                   OKTRegistrationResponse *registrationResponse,
                                                   NSError *error) {
    [expectation fulfill];
    ...
    [[OKTAuthorizationRequest alloc] initWithConfiguration:configuration
    ...
    self->_coordinator = [[OKTAuthorizationUICoordinatorNonInteractive alloc] init];

    [self certificationLog:@"Initiating authorization request: %@",
     [request authorizationRequestURL]];

    [OKTAuthorizationService presentAuthorizationRequest:request
                                       externalUserAgent:self->_coordinator
        callback:^(OKTAuthorizationResponse *_Nullable authorizationResponse,
                   NSError *error) {
      [auth_complete fulfill];
      XCTAssertNotNil(authorizationResponse);
      XCTAssertNil(error);

      OKTTokenRequest *tokenExchangeRequest = [authorizationResponse tokenExchangeRequest];
      [OKTAuthorizationService performTokenRequest:tokenExchangeRequest
                                          delegate:nil
                                          callback:^(OKTTokenResponse *_Nullable tokenResponse,
...
- (void)codeFlowWithExchangeExpectSuccessForTest:(NSString *)test {
  [self codeFlowWithExchangeForTest:test
                         completion:^(OKTAuthorizationResponse * _Nullable authorizationResponse,
                                      OKTTokenResponse * _Nullable tokenResponse,
                                      NSError *tokenError) {
...
- (void)codeFlowThenUserInfoForTest:(NSString *)test completion:(UserInfoCompletion)completion {
  ...
  NSArray<NSString *> *scope =
      @[ OKTScopeOpenID, OKTScopeProfile, OKTScopeEmail, OKTScopeAddress, OKTScopePhone ];
  [self codeFlowWithExchangeForTest:test
                              scope:scope
                         completion:^(OKTAuthorizationResponse * _Nullable authorizationResponse,
                                      OKTTokenResponse * _Nullable tokenResponse,
...
- (void)testRP_userinfo_bearer_header {
  ...
  [self codeFlowThenUserInfoForTest:testName
                         completion:^(OKTAuthState * _Nullable authState,
                                      NSDictionary * _Nullable userInfoDictionary,
...
- (void)testRP_userinfo_bad_sub_claim {
  ...
  [self codeFlowThenUserInfoForTest:testName
    ...
    XCTAssertNotNil(sub);
    OKTIDToken *idToken =
        [[OKTIDToken alloc] initWithIDTokenString:authState.lastTokenResponse.idToken];
